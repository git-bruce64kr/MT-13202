...
struct key scan_keyInterrupt ;
struct key mode_keyInterrupt ;
struct key select_keyInterrupt ;
# 697 "./mcc_generated_files/pin_manager.h"
void PIN_MANAGER_Initialize (void);
# 709 "./mcc_generated_files/pin_manager.h"
...
# 358 "./mcc_generated_files/tmr2.h"
void TMR2_ModeSet(TMR2_HLT_MODE mode);
# 393 "./mcc_generated_files/tmr2.h"
void TMR2_ExtResetSourceSet(TMR2_HLT_EXT_RESET_SOURCE reset);
# 422 "./mcc_generated_files/tmr2.h"
void TMR2_Start(void);
# 451 "./mcc_generated_files/tmr2.h"
void TMR2_StartTimer(void);
# 483 "./mcc_generated_files/tmr2.h"
void TMR2_Stop(void);
# 515 "./mcc_generated_files/tmr2.h"
void TMR2_StopTimer(void);
# 550 "./mcc_generated_files/tmr2.h"
uint8_t TMR2_Counter8BitGet(void);
# 585 "./mcc_generated_files/tmr2.h"
uint8_t TMR2_ReadTimer(void);
# 624 "./mcc_generated_files/tmr2.h"
void TMR2_Counter8BitSet(uint8_t timerVal);
# 663 "./mcc_generated_files/tmr2.h"
...
struct Timer0 {
    uint8_t chkTestCnt;
    uint8_t chkModeCnt;
    uint8_t chkSilentCnt;
    uint16_t chkBuzzerCnt;
    uint8_t T_chkTest;
    uint8_t T_chkMode;
    uint8_t T_chkSilent;
    uint8_t T_chkBattery;
    uint8_t T_chkBuzzer;
} ;
struct Timer0 thermoTimer;





uint8_t OperationCheckTimer = 0;
uint8_t blinkOn = 0;
uint16_t tmr0Cnt = 0;
uint16_t buzzerOn = 0;
uint16_t buzzerCnt = 0;
# 120 "./mcc_generated_files/tmr0.h"
void TMR0_Initialize(void);
# 149 "./mcc_generated_files/tmr0.h"
void TMR0_StartTimer(void);
# 181 "./mcc_generated_files/tmr0.h"
void TMR0_StopTimer(void);
# 217 "./mcc_generated_files/tmr0.h"
...
# 365 "./mcc_generated_files/tmr0.h"
...
# 1 "./mcc_generated_files/mylib.h" 1
...
# 57 "./mcc_generated_files/mylib.h"
 double Tobject = 0;
...
void main(void)
{
    ...
    TMR0_StartTimer();
    thermoTimer.T_chkSilent = 0;
    thermoTimer.T_chkBattery = 0;
    scan_keyInterrupt.falling = 0;
    scan_keyInterrupt.rising = 0;
    mode_keyInterrupt.falling = 0;
    storeTemp.disp = 0;
    runState = THERMOSTATE_NORMAL;

    if (0x5A != eeprom_read(0))
    {
        eeprom_write(0, 0x5A);
        eeprom_write(2, 0);
        eeprom_write(4, 0);
    ...
    while (1)
    {
        switch (runState)
        {
            case THERMOSTATE_INIT :
                if(scan_keyInterrupt.falling == 1)
                {
                    thermoTimer.T_chkMode = 1;
                    thermoTimer.chkModeCnt = 0;
                    thermoTimer.T_chkSilent = 1;
                    thermoTimer.chkSilentCnt = 0;
                }
                if(PORTAbits.RA5 == 1 && thermoTimer.T_chkMode == 1)
                {
                    runState = THERMOSTATE_NORMAL;
                    thermoTimer.T_chkMode = 0;
                }
                if(PORTAbits.RA5 == 0 && thermoTimer.T_chkMode == 2)
                {
                    thermoTimer.T_chkMode = 0;
                    runState = THERMOSTATE_SETMODE;
                }
                if(mode_keyInterrupt.falling == 1 )
                {
                    thermoTimer.chkTestCnt = 0;
                    thermoTimer.T_chkTest = 1;
                }
                if(thermoTimer.T_chkTest == 1 && PORTCbits.RC1 == 1)
                {
                    storeTemp.num++;
                    storeTemp.real = eeprom_read(10 + storeTemp.num );
                    storeTemp.disp = 1;
                    tempDisplay();
                    storeTemp.disp = 0;
                    thermoTimer.T_chkTest = 0;
                    mode_keyInterrupt.falling = 0;
                }
                if(thermoTimer.T_chkTest == 2 && PORTCbits.RC1 == 1)
                {
                    runState = THERMOSTATE_TEST;
                    thermoTimer.T_chkTest = 0;
                    mode_keyInterrupt.falling = 0;
                }
                if(thermoTimer.T_chkSilent == 2)
                {
...
# 210 "main.c"
            ...
            case THERMOSTATE_TEST :
                if(OperationCheckTimer == 1)
                {
# 244 "main.c"
                        OperationCheckTimer = 0;
                }
                break;
            default:
                break;
...
void setMode( void )
{
    ...
    setDisplay.mode0 = eeprom_read(2);
    setDisplay.mode1 = eeprom_read(4);
    setDisplay.mode2 = eeprom_read(6);
    setDisplay.toggle = 0;
    ...
    while(1)
    {
        ...
        switch(modeStep)
        {
            case 0 :
                if(eeprom_read(2 == 1 ))
                {
                    if( blinkOn == 1) LCDDATA1bits.S11C0 = 1;
                    else LCDDATA1bits.S11C0 = 0;
                }
                else
                {
                    if( blinkOn == 1) LCDDATA9bits.S26C1 = 1;
                    else LCDDATA9bits.S26C1 = 0;
                }
                if(setDisplay.toggle == 1)
                {
                    if(eeprom_read(2 == 1 )) eeprom_write(2, 0);
            ...
            case 1 :
                if(eeprom_read(4 == 0 ))
                {
                    if( blinkOn == 1) LCDDATA18bits.S00C3 = 1 ;
                    else LCDDATA18bits.S00C3 = 0;
                }
                else
                {
                    if( blinkOn == 1) LCDDATA14bits.S20C2 = 1;
                    else LCDDATA14bits.S20C2 = 0 ;
                }
                if(setDisplay.toggle == 1)
                {
                    if(eeprom_read(4 == 0)) eeprom_write(4,1);
                    else eeprom_write(4, 0);
            ...
            case 2 :
                if(eeprom_read(6) == 0)
                {
                    if( blinkOn == 1) LCDDATA12bits.S00C2 = 1;
                    else LCDDATA12bits.S00C2 = 0;
                }
                else
                {
                ...
                if(setDisplay.toggle == 1)
                {
                    if(eeprom_read(6) == 1) eeprom_write(6,0);
                    else eeprom_write(6, 1);
                    buzzer(1);
                    setDisplay.toggle = 0;
                }
                break;
        ...
        if(thermoTimer.T_chkMode == 1 && PORTAbits.RA5 == 1)
        {
            modeStep++;
            buzzer(1);
            if(modeStep == 4)
        ...
        if(thermoTimer.T_chkMode == 2 && PORTAbits.RA5 == 0)
        {
            runState = THERMOSTATE_INIT;
            thermoTimer.T_chkMode = 0;
            scan_keyInterrupt.falling = 0;
...
void modeDisplay(uint8_t mode)
{
    switch(mode)
    {
        case 0 :
            ...
            LCDDATA19bits.S10C3 = 1; LCDDATA13bits.S10C2 = 0; LCDDATA7bits.S10C1 = 0; LCDDATA1bits.S10C0 = 0; LCDDATA7bits.S11C1 = 1; LCDDATA19bits.S11C3 = 1; LCDDATA13bits.S11C2 = 1;

            LCDDATA19bits.S08C3 = 0; LCDDATA13bits.S08C2 = 1; LCDDATA7bits.S08C1 = 1; LCDDATA1bits.S08C0 = 0; LCDDATA7bits.S09C1 = 0; LCDDATA19bits.S09C3 = 0; LCDDATA13bits.S09C2 = 0;
            break;
        case 1 :

            LCDDATA19bits.S10C3 = 1; LCDDATA13bits.S10C2 = 0; LCDDATA7bits.S10C1 = 0; LCDDATA1bits.S10C0 = 0; LCDDATA7bits.S11C1 = 1; LCDDATA19bits.S11C3 = 1; LCDDATA13bits.S11C2 = 1;

            LCDDATA19bits.S08C3 = 1; LCDDATA13bits.S08C2 = 1; LCDDATA7bits.S08C1 = 0; LCDDATA1bits.S08C0 = 1; LCDDATA7bits.S09C1 = 1; LCDDATA19bits.S09C3 = 0; LCDDATA13bits.S09C2 = 1;
            LCDDATA15bits.S25C2 = 1; LCDDATA14bits.S20C2 = 1; LCDDATA12bits.S07C2 = 1; LCDDATA12bits.S02C2 = 1;
            break;
        case 2 :

            LCDDATA19bits.S10C3 = 1; LCDDATA13bits.S10C2 = 0; LCDDATA7bits.S10C1 = 0; LCDDATA1bits.S10C0 = 0; LCDDATA7bits.S11C1 = 1; LCDDATA19bits.S11C3 = 1; LCDDATA13bits.S11C2 = 1;

            LCDDATA19bits.S08C3 = 1; LCDDATA13bits.S08C2 = 1; LCDDATA7bits.S08C1 = 1; LCDDATA1bits.S08C0 = 1; LCDDATA7bits.S09C1 = 0; LCDDATA19bits.S09C3 = 0; LCDDATA13bits.S09C2 = 1;
            LCDDATA15bits.S25C2 = 1; LCDDATA14bits.S20C2 = 1; LCDDATA12bits.S07C2 = 1; LCDDATA12bits.S02C2 = 1;
            break;
        case 3 :
            ...
            LCDDATA19bits.S08C3 = 0; LCDDATA13bits.S08C2 = 1; LCDDATA7bits.S08C1 = 1; LCDDATA1bits.S08C0 = 0; LCDDATA7bits.S09C1 = 0; LCDDATA19bits.S09C3 = 1; LCDDATA13bits.S09C2 = 1;
            LCD_DisplayOff_NO_1_SYM06Num();
            LCD_DisplayOff_NO_2_SYM00Num();
            LCD_NO_3_SYM07Num(0);
            LCD_NO_4_SYM08Num(0);
            LCDDATA0bits.S02C0 = 1;
            break;
...
void tempDisplay(void)
{
    ...
    if(storeTemp.disp == 0)
    {
        ...
        if(20 <= Tobject && Tobject <= 42)
        {
            ...
            eeprom_write(10 + storeTemp.num, storeTemp.real);
            eeprom_write(10, storeTemp.num);
        }
    }
    else
    {
    ...
    LCDDATA0bits.S02C0 = 1;
    if (eeprom_read(2) == 1 ) LCDDATA1bits.S11C0 = 1;
    else LCDDATA9bits.S26C1 = 1;
    if (eeprom_read(4) == 0 ) LCDDATA18bits.S00C3 = 1;
    else LCDDATA14bits.S20C2 = 1;
    if (eeprom_read(6) == 0 ) LCDDATA12bits.S00C2 = 1;

    LCDDATA1bits.S09C0 = 1;
    ...
    {
        setColor(0);
        LCDDATA3bits.S25C0 = 1;
    }
    if(Tobject > 37.0 && Tobject <= 37.5) setColor(1);
    if(Tobject > 37.5) setColor(2);
...
adc_result_t getTempFromSensor(uint8_t channel)
{
    ...
    if(channel == 0) return(ADC_CalcAvg(channel_ANE0));
    else return(ADC_CalcAvg(channel_ANE1));
...
static uint16_t battMaxLevel = 1920;
...
uint8_t battMeasure(void)
{
    ...
    batt = (pre_percent*((float)2.495 /(float)4096)) ;
...
# 742 "main.c"
    ...
    if(percent >= 75)
    {
        LCDDATA0bits.S00C0 = 1; LCDDATA1bits.S11C0 = 1; LCDDATA15bits.S26C2 = 1; LCDDATA21bits.S26C3 = 1;
    }
    if(percent >= 40 && percent <75)
    {
        LCDDATA0bits.S00C0 = 1; LCDDATA1bits.S11C0 = 1; LCDDATA15bits.S26C2 = 1; LCDDATA21bits.S26C3 = 0;
    }
    if(percent >= 10 && percent < 40)
    {
        LCDDATA0bits.S00C0 = 1; LCDDATA1bits.S11C0 = 1; LCDDATA15bits.S26C2 = 0; LCDDATA21bits.S26C3 = 0;
    }
    if(percent > 0 && percent < 10)
    {
        LCDDATA0bits.S00C0 = 1; LCDDATA1bits.S11C0 = 0; LCDDATA15bits.S26C2 = 0; LCDDATA21bits.S26C3 = 0;
    }
    if( percent == 0)
    {
        if(blinkOn == 1)
...
void testDisplay(void)
{
    LCDDATA18bits.S00C3 = 1; LCDDATA14bits.S20C2 = 1; LCDDATA9bits.S26C1 = 1; LCDDATA0bits.S02C0 = 1; LCDDATA1bits.S11C0 = 1;
    LCDDATA12bits.S00C2 = 1; LCDDATA1bits.S09C0 = 1; LCDDATA3bits.S25C0 = 1; LCDDATA2bits.S20C0 = 1;
    {
    LCDDATA0bits.S00C0 = 1; LCDDATA1bits.S11C0 = 1; LCDDATA15bits.S26C2 = 1; LCDDATA21bits.S26C3 = 1;
    }
